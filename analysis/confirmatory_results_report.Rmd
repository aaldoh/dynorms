...
shorttitle        : "Confirmatory hypotheses"
author: 
  - name          : "Alaa AlDoh"
    affiliation   : "1"
...
```{r, results = 'asis'}
cortable %>%
  kable(format = "html",col.names = c("Measure", "$M$", "$SD$", "$\\alpha$", 1:5), escape = F) %>% 
  add_header_above(c(" " = 4, "Correlations" = 5)) %>%
  kable_styling()
```
...
```{r, results = "asis"}
apa_table(interest_desc, caption = "Meat consumption by condition", escape = T)
```
