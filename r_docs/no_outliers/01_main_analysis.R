...
# Data Overview ---------------------------
age_desc <- apply(no_out[27], MARGIN = 2, function(x) c(min = min(x), max = max(x), avg = mean(x), sd = sd(x))) ## age
gender_freq <- 100 * prop.table(table(no_out$GENDER)) ## gender
cron <- apply(matrix(12:20, ncol = 3), 2, function(x) printnum(cronbach(no_out[x])$alpha))
measured_vars <- no_out[, c(11, 28:31, 33, 25)]
...
# Randomization check ---------------------------
demobycond <- group_by(no_out, condition) %>%
...
age_stat <- apa_print(aov(AGE ~ condition, no_out))
pol_check <- paste(demobycond$mean_POLITICS, demobycond$sd_POLITICS, sep = ' $\\pm$ ')
pol_stat <- apa_print(aov(POLITICS ~ condition, no_out))
...
gender_stat <- apa_print(chisq.test(no_out$condition, no_out$GENDER), n = nrow(no_out))
...
nation_stat <- apa_print(chisq.test(no_out$condition, no_out$UKNATION), n = nrow(no_out))
...
# H1 interest ---------------------------
outcomes_desc <- group_by(no_out, condition) %>%
  summarise(n = n(),
...
ls_interest <- lm(INTEREST ~ condition, no_out) %>%
...
# full regression
pol.interest_out <- apa_print(summary(lm(INTEREST ~ POLITICS, no_out)))
gen.interest_out <- apa_print(t.test(INTEREST ~ genderbi, no_out))
demoreg.out <- summary(lm(INTEREST ~ conditionbi + genderbi + POLITICS + conditionbi:genderbi + conditionbi:POLITICS, no_out)) %>%
  apa_print()
...
# H2 ---------------------------
future <- group_by(no_out, condition) %>%
  summarise(n = n(),
            across(c("PERCEPTCHANGE_r", "PRECONFORMITY", "comb_future"), list(mean = mean, sd = sd),
                   .names = "{.fn}_{.col}"), .groups = "rowwise")

# Future consumption
ls_change <- lm(PERCEPTCHANGE_r ~ condition, no_out) %>%
  emmeans(., "condition", contr = contrasts) %>%
  .$contrasts %>%
  c(summary(.), apa_print(.))
...
# Preconformity
# contrasts
ls_preconformity <- lm(PRECONFORMITY ~ condition, no_out) %>%
  emmeans(., "condition", contr = contrasts) %>%
  .$contrasts %>%
  c(summary(.), apa_print(.))
...
# combined
# contrasts
ls_comb_future <- lm(comb_future ~ condition, no_out) %>%
  emmeans(., "condition", contr = contrasts) %>%
  .$contrasts %>%
  c(summary(.), apa_print(.))
...
# Secondary analyses ---------------------------
secondary <- group_by(no_out, condition) %>%
  summarise(n = n(),
...
no_out %>%
  ...
  saveRDS(., file = here("data/blav_data_no_out.rds"))

saveRDS(no_out, file = here("data/no_out.rds"))
