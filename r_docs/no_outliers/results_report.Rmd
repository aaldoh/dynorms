---
title: "Results Notebook (no outliers)"
floatsintext : yes
output:
...
load(here("figures/exp_plots_out.RData"))
load(here("data/exp_results_out.RData"))
load(here("data/unregistered_out.RData"))

knitr::opts_chunk$set(echo=F, warning = F, message = F, results = "asis", fig.path = "no_outliers/figures/")
```
...
```{r h3-trace, fig.show='asis', fig.cap="Traceplots of regression parameters"}
...
```{r h4-trace, fig.show='asis', fig.cap="Traceplots of regression parameters"}
...
continuous_mod.out %>% 
  kable(caption = "Posterior results for moderation model using age as continuous variable (H4)", row.names = F, escape = F) %>%
  kable_styling() %>%
...
```{r h5-trace, fig.show='asis', fig.cap="Traceplots for estimated regression parameters"}
...
```{r moderation-attit}
apa_table(attitude_mod.out$table, caption = "Exploring the effect  moderation of demographic variables on attitudes", escape = F)
```

```{r moderation-intent}
apa_table(intent_mod.out$table, caption = "Exploring the effect  moderation of demographic variables on intention", escape = F)
```

```{r h1-fullreg, results='markup'}
apa_table(demoreg.out$table, caption = "Exploring the effect  moderation of demographic variables on interest", escape = F)
```
...
```{r mediation}
kable(med.table, escape = F, col.names = c("Parameter", "$M$", "$SD$", "Lower PPI", "Upper PPI", "Rhat", "Prior"), caption="Exploring mediating effect of attitude on intentions/expectations") %>%
  kable_styling()
```
...
## Codebook
```{r codebook}
codebook::compact_codebook(no_out) 
```
