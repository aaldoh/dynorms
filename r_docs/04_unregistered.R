...
med.fit <- bsem(med.mod, data = blav_data, target = "stan", seed = 2019)
...
medpercept.fit <- bsem(medpercept.mod, data = blav_data, target = "stan", seed = 2019)
medpercept.out <- summary(medpercept.fit, fit.measures = T)
medpercept.pam <- parameterEstimates(medpercept.fit)

# Moderation ---------------------------
attitude_mod.out <- summary(lm(attitude_mean ~ conditionbi + genderbi + POLITICS + conditionbi:genderbi + conditionbi:POLITICS, clean)) %>%
  apa_print()

intent_mod.out <- summary(lm(expintent_avg ~ conditionbi + genderbi + POLITICS + conditionbi:genderbi + conditionbi:POLITICS, clean)) %>%
  apa_print()


save(h1.pwr, med.table, medpercept.out, intent_mod.out, attitude_mod.out, file =  "data/unregistered.RData")
