...
h3.mcmc_intervals <- bind_rows(lapply(h3.mcmc, function(x)
  ...
  mutate(Model = as.factor(rep(c("1: Uninformative", "2: Weakly informative", "3: Moderately informative"), each = 3)))

h3.density <- bind_rows(lapply(h3.mcmc, function(x)
  mcmc_areas_data(x, regex_pars = "^bet", prob = 0.8, prob_outer = 0.99, point_est = "mean"))) %>%
  mutate(Model = as.factor(rep(c("1: Uninformative", "2: Weakly informative", "3: Moderately informative"), each = 6211)))
...
h4.mcmc_intervals <- bind_rows(lapply(h4.mcmc, function(x)
  mcmc_intervals_data(x, regex_pars = "^bet", point_est = "mean", prob = 0.8, prob_outer = 0.95))) %>%
  mutate(Model = rep(c("1: Uninformative", "2: Weakly informative", "3: Moderately informative"), each = 9),
         Age = ifelse(parameter %in% c("bet_sign[1]", "bet_sign[2]", "bet_sign[3]"), "Young adults",
